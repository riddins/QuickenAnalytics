...
class QifParser(object):
    ...
    def parse(cls_, file_handle, date_format=None):
        ...
        for chunk in chunks:
            ...
            if first_line == '!Type:Cat':
                ...
                raise QifParserException('Header {0} not reconized on line {1}'.format(first_line, chunk))
    ...
    def parseSecurity(cls_, chunk):
        ...
        for line in lines:
            if not len(line) or line[0] == '\n' or \
                ...
                curItem.ticker = line[1:]
            elif line[0] == 'T':
                curItem.security_type = line[1:]
        return curItem

    @classmethod
    def parsePrice(cls_, chunk):
        """
        """
        curItem = Price()
        lines = chunk.split('\n')
        for line in lines:
            if not len(line) or line[0] == '\n' or \
                ...
                vals = line.split(',')
                curItem.ticker = vals[0].strip('"')
                curItem.price = vals[1]
                curItem.date = vals[2].strip('"')
    ...
    def parseCategory(cls_, chunk):
        ...
        for line in lines:
            if not len(line) or line[0] == '\n' or line.startswith('!Type'):
                ...
                curItem.expense = True
            elif line[0] == 'I':
                curItem.income = True
                curItem.expense = False  # if ommitted is True
            elif line[0] == 'T':
                ...
                curItem.tax_schedule_amount = line[1:]
            elif line[0] == 'N':
                curItem.name = line[1:]
        return curItem
    ...
    def parseQifDateTime(cls_, qdate):
        ...
        return datetime.strptime(iso_date, '%Y-%d-%m')
